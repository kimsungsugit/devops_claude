# /app/gui/gui_utils.py
# -*- coding: utf-8 -*-
# GUI와 Workflow를 연결하는 핵심 브리지 (Logic Layer)
...
def save_history(paths: Dict[str, Path], summary: Dict[str, Any] | None, avg_complexity: Optional[float]) -> None:
    ...
    total_issues = cpp_issues + tidy_issues
...
# -----------------------------
# 파이프라인 실행 (GUI → Workflow)
# -----------------------------

def run_pipeline(cfg: Dict[str, Any], status_box, progress_bar, log_callback: Optional[Callable] = None) -> int:
    """
    Streamlit GUI에서 호출하는 메인 엔트리
    - cfg: gui_sidebar 설정
    - status_box, progress_bar: Streamlit 위젯
    - log_callback: 화면 출력용 콜백 (app.py에서 주입)
    """
    ...
    try:
        ...
        if report_dir.exists():
            ...
            if tmp.exists():
                shutil.rmtree(tmp)
            shutil.copytree(report_dir, tmp)
            tmp.rename(backup_dir)
    except Exception as e:
        print(f"[WARN] report backup failed: {e}")
    ...
    try:
        exit_code = workflow.run_cli(
            ...
            do_build_and_test=cfg.get("do_build", False),
            do_coverage=True,
...
def start_pipeline_async(cfg: Dict[str, Any], paths: Dict[str, Path]) -> Dict[str, Any]:
    """Launch pipeline in a separate process for non-blocking GUI."""
    report_dir = paths["REPORT"]
...
def load_summary_with_fallback(paths: Dict[str, Path]) -> Tuple[Dict[str, Any], bool]:
    ...
    try:
        ...
        print(f"[WARN] Failed to load real summary: {e}")
        summary = {}
