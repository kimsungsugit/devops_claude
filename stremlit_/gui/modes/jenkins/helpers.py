...
def _sanitize_relpath(rel: str) -> str:
    ...
    NOTE: Do not use for filesystem writes; use _safe_artifact_path instead.
    """
    rel = (rel or "").replace("\\", "/").lstrip("/")
    # keep only a basic normalization here (no traversal protection)
    return rel
...
def _safe_artifact_path(build_root: "Path", rel: str) -> "Path|None":
    ...
    - Reject absolute paths, drive letters, home (~)
    - Reject any '..' traversal
    - Ensure resolved path stays inside build_root
    """
    try:
...
def _guess_project_root(build_root: Optional[Path], reports_dir: Optional[Path]) -> str:
    ...
    if rdir and (rdir / "analysis_summary.json").exists():
        try:
            summary = gui_utils.load_json(rdir / "analysis_summary.json", default={})
        except Exception:
            summary = {}
    ...
    # jenkins_scan.json hint: relative roots under build_root
    jscan = None
    if isinstance(summary, dict) and isinstance(summary.get("jenkins_scan"), dict):
        ...
        try:
            jscan = gui_utils.load_json(rdir / "jenkins_scan.json", default={})
        except Exception:
            jscan = None
