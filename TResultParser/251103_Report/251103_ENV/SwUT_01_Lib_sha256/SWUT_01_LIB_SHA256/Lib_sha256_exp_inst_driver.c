...
/*vcast_header_expansion_start:Lib_sha256_driver_prefix.c*/
/*vcast_header_expansion_end*/
...
extern int vCAST_ITERATION_COUNTERS [3][17];
      ...
      if (vCAST_COMMAND == vCAST_ALLOCATE && vcast_proc_handles_command(1)) {
        ...
        if (VCAST_FIND_INDEX() == -1) {
          ...
          *VCAST_TI_9_17_memory_ptr = (void*)VCAST_malloc(VCAST_TI_9_17_array_size*(sizeof(unsigned  )));
          memset((void*)*vcast_param, 0x0, VCAST_TI_9_17_array_size*(sizeof(unsigned  )));
        }
      } else if (vCAST_VALUE_NUL == vCAST_true && vcast_proc_handles_command(1)) {
        if (VCAST_FIND_INDEX() == -1)
          *vcast_param = 0;
      ...
      if (vCAST_COMMAND == vCAST_ALLOCATE && vcast_proc_handles_command(1)) {
        ...
        if (VCAST_FIND_INDEX() == -1) {
          ...
          *VCAST_TI_9_18_memory_ptr = (void*)VCAST_malloc(VCAST_TI_9_18_array_size*(sizeof(SHA256_CTX  )));
          memset((void*)*vcast_param, 0x0, VCAST_TI_9_18_array_size*(sizeof(SHA256_CTX  )));
        }
      } else if (vCAST_VALUE_NUL == vCAST_true && vcast_proc_handles_command(1)) {
        if (VCAST_FIND_INDEX() == -1)
          *vcast_param = 0;
...
void VCAST_TI_RANGE_DATA_9 ( void ) {
  ...
  vectorcast_fprint_unsigned_long (vCAST_OUTPUT_FILE,0 );
  vectorcast_fprint_string (vCAST_OUTPUT_FILE,"\n" );
  vectorcast_fprint_unsigned_long (vCAST_OUTPUT_FILE,(0xffffffff / 2) );
  vectorcast_fprint_string (vCAST_OUTPUT_FILE,"\n" );
  vectorcast_fprint_unsigned_long (vCAST_OUTPUT_FILE,0xffffffff );
  vectorcast_fprint_string (vCAST_OUTPUT_FILE,"\n" );
  vectorcast_fprint_string (vCAST_OUTPUT_FILE, "NEW_ARRAY\n" );
...
/*vcast_header_expansion_start:Lib_sha256_uc.c*/
void vCAST_VALUE_USER_CODE_9(int vcast_slot_index ) {
  {
  }
}
