...
--                             The size of the buffer is controlled by macro"
...
#if !defined (VCAST_GH_INT_178B) && !defined (VCAST_DO_NOT_INCLUDE_STDIO)
#include <stdio.h>
#endif
...
void VCAST_ida_allocate_memory()
{
   ...
   */
   inputDataArrayStart =  (const char*)VCAST_malloc(vcastFileSystemSize);
#endif
...
void vcast_clear_output_buffer (void) {
   /* Use memset so that RSPs which dump the entire buffer have clean output */
...
int StartsNewFile(const char * line){
  if (VCAST_strncmp(line, "*FILE:", 6) == 0)
    return vCAST_true;
...
Parameters: fileData - the FileData structure to get a line from
            ...
            advanceOffset - If true, update the buffer position after read
Description: This function returns the line at the current location in
the FileData structure.
WARNING: String must contain a newline for reliable operation
  *************************************************************************/
char * GetCurrentLine(struct FileData * fileData, char *uline, int maxlen, int advanceOffset){
  const char* ptr;
  ...
  ptr = (inputDataArrayStart + lInitialOffset+lCurrentOffset);
  while ( *ptr != '\n' )
  {
    if (++len < maxlen)
    {
...
static void  AddLineToInputDataArray(const char *lineContents, 
  ...
  unsigned char lb;
  unsigned char hb;
  const char *linePtr = lineContents;
...
void AddNewLine(struct FileData * fileData, 
  ...
  /* Add to the offset the length of the line including the linefeed */
  currentIndexIntoDataArray += (lineLen) + 1;
}
...
void vcast_probe_assert (const char *msg, int condition)
{
    ...
    vectorcast_fprint_string_with_cr (VCAST_EXP_FILE, msg?msg:"(description not provided)");
...
{
    if (!VCAST_EXP_FILE || !vCAST_PROBE_POINTS_AS_EVENTS ||
        !vcast_is_in_driver)
      return;

    vCAST_UC_WRITE_EXPECTED("", msg?msg:"(description not provided)", 1, "");

    vCAST_SET_HISTORY( 1018, 999 );
}
#endif
...
{
  char line_str[32];
  vectorcast_signed_to_string(line_str, line);

  vectorcast_fprint_string_with_cr(VCAST_EXP_FILE, "-- AssertEvent");
  vectorcast_fprint_string_with_cr(VCAST_EXP_FILE, (result ? "1" : "0"));
  vectorcast_fprint_string_with_cr(VCAST_EXP_FILE, expr);
  vectorcast_fprint_string_with_cr(VCAST_EXP_FILE, lhs);
  vectorcast_fprint_string_with_cr(VCAST_EXP_FILE, rhs);
  vectorcast_fprint_string_with_cr(VCAST_EXP_FILE, line_str);
}
...
int vectorcast_readline(char *vcast_buf, int fpn) {
   ...
   vcast_buf[0] = '\0';
   vectorcast_fgets(vcast_buf, VCAST_MAX_STRING_LENGTH, fpn);
...
int vectorcast_feof(int fpn) {
  ...
  return TRACE32_TERM_EOF(vcast_file_descriptor[fpn].handle);
   ...
   if (GetCurrentLine(file, line, VCAST_MAX_STRING_LENGTH, 0) != 0)
     i = 0;
   else
     i = 1;
...
#elif defined(VCAST_USE_SETLINEBUF) || defined(VCAST_USE_SETVBUF) || defined(VCAST_USE_SETUNBUF)
...
#define VCAST_SETVBUF_MODE _IOLBF
   ...
   setvbuf( fd->fp, NULL, VCAST_SETVBUF_MODE, 0 );
#endif
...
#if defined(__cplusplus)
   for (int file_index = 0; file_index < VCAST_MAX_FILES; ++file_index){
...
int vectorcast_fopen( const char *filename,  const char *mode) {
  ...
  if (VCAST_strcmp(mode, "r") == 0){
   ...
   /* Position the pointer at specified offset from the beginning of inputDataArray */
   line = (inputDataArrayStart + lInitialOffset);
   end_ptr = line;
   while ( *end_ptr != '\n' )
     ++end_ptr;
   len = end_ptr - line + 1;
...
#ifdef VCAST_STDIO
  struct FileData * file = GetFile(fpn);
  char* ret = GetCurrentLine(file, line, maxline,1);
...
vCAST_boolean vCAST_PROBE_POINTS_AS_EVENTS;   /* enable events for probe points or not */
...
#define VCAST_DIVISOR_ZERO VCAST_globalZero
#endif
vCAST_long_double VCAST_GET_QUIET_NAN ()
      { return VCAST_FLOAT_ZERO/VCAST_DIVISOR_ZERO; }
vCAST_long_double VCAST_GET_NEGATIVE_INFINITY ()
      { return (-VCAST_FLOAT_ONE)/VCAST_DIVISOR_ZERO; }
