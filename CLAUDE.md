...
- **ESLint (2026-07-20 신설)**: `frontend-v2` 에 flat config·의존성이 **아예 없어** 훅이 매 편집마다 `npx eslint` ERROR 만 냈다 = JSX lint 게이트가 한 번도 동작한 적 없음. `eslint.config.js`(테스트 전역 블록 필수 — 없으면 `no-undef` 2,000건+) + `scripts/eslint_ratchet.py` 신설. **실측 backlog = error 35 / warning 16**(2026-09-02 실측, `frontend-v2/src` 17파일 — 신설 시점 101/36 → 73/18 → 2026-08-19 정리 35/18 → 현재 35/16). 남은 51건은 **전부 판단 축**이다: react-hooks 계열 42(exhaustive-deps 16·set-state-in-effect 15·immutability 5·refs 3·purity 2·preserve-manual-memoization 1 — deps 를 기계적으로 채우면 무한 루프·stale closure) + react-refresh/only-export-components 7(파일 분리 필요) + no-unused-vars 2 → ratchet. **변경 라인은 error·warning 둘 다 차단**(severity 1도 — 레거시만 면제): warning 이 대부분 `react-hooks/exhaustive-deps`(X2 stale closure)라, 비차단이면 유일한 자동 X2 검사가 아무것도 안 막는다. npx 대신 `_hook_env.project_eslint()` 로 **로컬 바이너리 직접 해석**(npx 는 미설치 시 레지스트리에서 받아와 훅이 조용히 네트워크에 의존했다)
