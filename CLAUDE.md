...
- **CI**: GitHub Actions(`ci.yml` = syntax-check + unit-tests + frontend-tests + **lint**[ruff_ratchet 변경라인 + SKILL.md frontmatter]) + GitLab CI(stages: lint[syntax + ruff-frontmatter]/test/frontend). **검증 전용 — 배포 스텝 없음**
...
- **`.githooks/pre-commit` (3단계 게이트, 2026-07-18 ruff·frontmatter 승격)**: syntax(py_compile) → **`ruff_ratchet.py --cached`(변경라인 신규위반) + `check_skill_frontmatter.py`** → pytest(900s). ruff·frontmatter 는 이전엔 PostToolUse 훅에서만 발화해 `--no-verify`·외부 에디터·auto_commit_push 가 우회했다 → 커밋 시점·CI(`lint` job)로 승격. 저장소 ruff backlog(1072건/247파일)라 **파일 전체가 아닌 변경 라인 ratchet**(`scripts/ruff_ratchet.py`). ⚠rename 은 `-M`+**pathspec 없이**라야 감지(pathspec 이 old 배제→파일 전체 폭주). ruff 자체 실패(rc≠0+빈 stdout)는 `[]`→clean 이 아니라 **DISABLED(fail-closed)**. 로컬은 DISABLED 경고후허용/CI 는 실패(의도적 비대칭)
