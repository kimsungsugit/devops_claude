...
## 왜 ``Path`` 검사(ASYNC240 잔여 35건)는 일부러 안 잡는가

불변식 2 는 **무거운 생성 호출**만 본다. ``async`` 핸들러 안에 남아 있는 맨
``Path.exists()/is_file()/resolve()`` 는 대상이 아니다 — 고치는 게 이득이 아니어서다.

실측 (2026-08-05, cloudium 모드, ``config/scm_registry.json`` linked_docs 30건 전량):

    resolve + exists + is_file  →  중앙 0.43ms / p95 0.53ms / 최대 0.81ms
    한 요청이 30건을 전부 돌아도  →  합계 13.1ms
    UNC(``\\\\host\\share``) 형태 →  0건

Cloudium DRM 이 백엔드 프로세스를 **즉시** 거절하기 때문에(PermissionError 0.1ms)
네트워크 왕복 자체가 일어나지 않는다. 즉 이 35건을 스레드로 옮겨 얻는 것은 약 13ms
이고, 대신 프로덕션 생성 핸들러에 변경 위험을 더한다. 수 분짜리 AI 호출·문서 빌드를
옮긴 것과는 **비용 규모가 세 자릿수 다르다**.

⚠ 이 판단이 뒤집히는 조건은 하나다 — 레지스트리에 **UNC 경로**(``\\\\10.x.x.x\\share``)
가 들어오는 것. 도달 불가 호스트의 UNC 는 실측 42초까지 걸린다. 그때는 이 문단의
근거가 무효이므로 다시 재고 결정할 것. "정적분석이 35건이라 하니 고친다" 는 이유로
건드리지 말 것 — 그건 측정이 아니라 도구 출력을 따라가는 것이다.
"""
