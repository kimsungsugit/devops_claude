...
⚠ **MID는 대조하지 않는다.** (1)과 (2)의 MID 정의가 실제로 다르다:
   ...
   더구나 `_TYPE_BOUNDARIES["uint32_t"]["mid"]`는 `2**15`(=32768)로, uint32의 중앙이라면
   `2**31`이어야 할 값이다. 이건 **실 문서 산출값이라 여기서 고치지 않는다**(고치면 생성되는
   xlsm이 바뀐다 — 별도 판단 필요). 누군가 "정합을 맞춘다"며 표시 MID를 생성기 값에 끌어다
   맞추지 않도록 이 사유를 여기 남긴다.
