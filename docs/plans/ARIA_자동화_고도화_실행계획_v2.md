...
| ~~**21**~~ | ~~이름맵이 해석 루프를 통째로 건너뛴다~~ | ✅ **완료** — §5-15. C2(재결합, `21df0ca`) + C3(키 규칙 단일화, `fc246d7`). C4(미러)는 C2 이후 **구조적 no-op** 이라 손대지 않았고, orphan 축은 by_name 이 진부분집합이라 **구조적으로 닫혔다** |
...
### C3 — orphan 을 재려다 더 큰 것이 나왔다 (커밋 `fc246d7`)

**원래 질문**(by_name 에만 있는 항목이 해석 루프를 건너뛰는 규모)에 답하려고 라우터를
읽다가, `function_details_by_name` 을 만드는 코드가 **4곳에 복제**돼 있고 둘이 **키 규칙을
틀렸다**는 걸 발견했다:

| 위치 | 키 규칙 | |
|---|---|---|
| `uds_generator.py::_put_by_name` | `.strip().lower()` | 정본 |
| `jenkins.py` | `.strip().lower()` | ✓ |
| `local.py` | `.strip()` | ✗ **원형 대소문자** |
| `tools/generate_uds_local.py` | `.strip()` | ✗ **원형 대소문자** |

**조회는 전부 소문자다** — `docx_builder` 13곳 · `code.py:126` · `test_gen.py:32` ·
`uds_generator` 4곳. 즉 로컬 경로가 만든 맵은 대문자가 든 함수를 **하나도 못 찾는다.**
실측: 표본 350개 중 **267개(76.3%)** 가 대문자를 포함한다.

잃던 것 — 호출자/피호출자 **시그니처**(프로토타입 대신 맨 이름으로 degrade) ·
SwCom **파일 경로** · `/api/code`·`test_gen` 함수 본문 조회 **404**. 전부 조용하다.

→ `report_gen/utils.py` 에 `function_name_key()` / `build_function_details_by_name()`
단일 출처. 라우터 3곳 + 정본 + `rejoin_function_maps` 가 전부 이걸 쓴다.

⚠ **순서가 중요했다.** 이 수정은 C2 **이후에만** 안전하다 — C2 전에 로컬 키를 소문자로
맞추면 렌더러가 로컬에서도 by_name 을 적중시켜 **enrich 안 된 사본을 그리게** 되어
결함을 두 경로로 확산시킨다(계획서가 미리 경고해 둔 그대로였다).

**원래의 orphan 질문은 이로써 구조적으로 닫혔다** — 4개 빌더 전부 `function_details` 에서
by_name 을 만들므로 by_name 은 **진부분집합**이다(동명은 last-wins 로 줄어들 뿐 늘지 않는다).
남는 유일한 orphan 원천은 `docx_builder:1812-1840` 시그니처 파싱 폴백인데, 그건
`by_name` 이 비었을 때만 도는 경로라 `function_details` 도 비어 있어 해석 루프 자체가
할 일이 없다. **없는 결함에 코드를 더하지 않는다.**
...
- **G3·G5·G6·G7** (검토 기록 쓰기) — §6 22행. 착수 조건은 정정됐다(위).
- `config/scm_registry.json` 의 `hdpdm01.linked_docs.uds` 가 **KJPDS02 문서**를 가리킨다.
  프로젝트별 템플릿 바인딩 착수 전에 정리 필요.
- 보안 표면 3건은 **§5-17 에서 전부 처리**했다.

---

## 5-17. 보안 표면 3건 — ✅ 완료 (2026-08-04)

사용자 지시 *"남은 3건 진행해"* + 결정 4건. 4커밋. §5-15/§6-1 이 "보안 표면 변경이라
사용자 결정" 으로 미뤄 둔 것 전부다.

### 방법 — 재현하지 못한 위험은 위험으로 세지 않았다

측정 4 + **반증 4** 병렬(오류 0). 규약: *"악용 경로는 **재현**하라. 코드를 읽고 '가능해
보인다' 는 부족하다."* 전부 FastAPI TestClient 로 실제 요청을 보냈고, 생성 파일은 즉시
삭제·부재 확인했다.
...
| 표면 | 재현된 것 |
|---|---|
| `/api/local/editor/*` | 비관리자 헤더 하나로 홈·`C:\Windows\Temp`·**Windows 시작프로그램 폴더**에 파일 생성(지속성), `backend/routers/*.py` 코드 주입, `.env` **2,165B 읽기**(JWT_SECRET·BOOTSTRAP_ADMIN_PASSWORD), `quality.sqlite` 읽기·덮어쓰기 |
| `X-User` 폴백 | `X-User: <admin 이름>` 한 줄로 admin 라우트 **27개** 통과 — `/api/swut/sutr/build` 등 **ISO 26262 evidence 생성기** 포함. `/api/auth/me` 가 admin 계정명 oracle |
| `/api/scm/*` | 비관리자가 registry 생성·수정·삭제·문서연결 **전부 200**, `scm_url` 을 공격자 호스트로 재지정, cloudium 읽기 경계 라이브 확장, 임의 URL 아웃바운드 |
| CORS `*` | 임의 웹페이지가 `localhost:9000` 응답을 판독(drive-by) — 위 `.env` 유출의 실제 전달 경로 |

⚠ **traversal 가드는 이미 있었고 정상 동작했다**(`../` 3종 전부 차단). 결함은 traversal 이
아니라 **base 지정**이었다 — `rel_path` 만 검사하고 root 는 body 를 믿었다. 같은 파일의
`open-file`/`read-abs`/`open-folder` 는 **이미** 확정을 하고 있었다(읽기전용은 잠기고
쓰기만 열린 비대칭).
...
| 결정 | 조치 | 커밋 |
|---|---|---|
| 폴백을 **끈다** | `.env` = 0(사용자 PC, 커밋 대상 아님) + best-effort 비대칭 제거 + 기동 경고 + `allowed_users` fail-open 정직화 + `.env.example` 명시 `=0` | `1b6bb99` |
| **20곳 전부 잠금** | `confine_request_root()` 단일 헬퍼(허용 루트 확정 + 민감 경로 거부) + `require_admin` | `346cc72` |
| SCM **쓰기만** admin | register/update/delete/link-docs/test 5종. 읽기는 UI 초기 로드가 의존해 개방 유지 | `2a50805` |
| LAN 접속 **유지** | CORS 를 알려진 origin 으로. bind 0.0.0.0 유지 — 프론트를 백엔드가 같은 오리진에서 서빙하므로 **LAN 은 same-origin 이라 안 깨진다** | `075fcc5` |

`.env` 한 줄만으로 세 표면이 전부 401 이 됐다. 그래도 코드 심층방어를 넣은 이유는
`.env` 가 untracked 라 **한 줄로 되살아나기** 때문이다 — 최악 가정(폴백 되켜짐 + admin
신원)에서도 전부 403 임을 확인했고, 저장소 안 정상 쓰기는 200 으로 살아 있다.

### 곁가지 — 검출기가 구조적으로 못 보던 축

cloudium path-키 회귀 테스트가 `.get("키")`·`Form(` 만 봐서 **Pydantic body 를 통째로
못 봤다**(실측 `project_root detected? False`). 그래서 이 20곳은 cloudium 모드에서도
게이트 없이 통과했고 회귀 테스트는 **초록**이었다. 축을 넓히자 **31개 키**가 드러났다.

전부를 즉시 게이트에 올리지 **않았다** — 그 순간 cloudium 모드 문서 생성이 막힐 수 있는데
worker 없이는 검증할 수 없고, **검증 못 하는 게이트 확장은 이 저장소가 금지한 부류**다.
`ruff_ratchet` 방식으로 **기존은 명시 부채, 신규는 차단**. 보이지 않던 사각이 **세어지는
목록**이 된 것이 소득이고, 각 키를 올리는 건 cloudium 실환경 검증과 함께 갈 별건이다.

### ⚠ 이 라운드에 사고를 두 번 냈다 (규약으로 승격)

① admin **대조군**이 라이브 앱에 진짜 요청을 보내 `DELETE /api/scm/delete/hdpdm01` 이
   **운영 registry 엔트리를 실제로 지웠다**(커밋본에서 복원).
② 경로 격리 픽스처를 넣었는데, **그 픽스처를 무력화하는 뮤테이션**을 돌리는 동안 같은
   삭제 + 쓰레기 엔트리 유입이 또 일어났다(스냅샷에서 복원).

> **게이트를 검증하려고 게이트 뒤의 부작용을 실행하지 마라.** 경로만 갈아 끼우는 격리는
> 뮤테이션·리팩터에 취약하다. **부작용 함수 자체를 스텁**하고, 뚫려도 파일이 남지 않게
> autouse 안전망을 두고, **스텁이 실제로 호출됐는지**를 값으로 확인한다(파일 무변조만
> 보면 "요청이 422 로 죽어서" 도 통과 — 공허 통과). 뮤테이션 하네스에도 스냅샷·복원을 둘 것.

같은 라운드에 뮤테이션 하네스의 **복원 쓰기가 Windows 에서 `OSError(Errno 22)` 로 실패**해
소스가 변형된 채 남을 뻔한 일도 있었다(즉시 발견·복원). 하네스에 재시도 + 복원 검증 추가.

### 검증

| 축 | 값 |
|---|---|
| 뮤테이션 | **30/31 killed**(6+9+8+7). 생존 1건은 **정당**: SCM autouse 안전망은 동작이 아니라 최후 복원 장치라, 1차 방어(부작용 스텁)가 살아 있으면 발화하지 않는다 |
| 백엔드 스위트 | 5,424 → **5,514 passed / 1 skipped** (`-n auto` 200초) |
| 신규 테스트 | 88건 |

### 잔여

- Pydantic body 축 **31키 backlog** — cloudium 게이트 등재는 실환경 검증과 함께.
- 폴백 **코드 자체**는 남겼다(사용자 결정은 "끈다" 였지 "제거한다" 가 아니다). 되켜지면
  기동 경고가 뜬다.
