...
- ⬜ **pre-commit 900s 예산 임계 근접** — `tests/unit/` 이 2026-07-17 3,486개/281초 → 2026-07-29 4,638개/**590초**.
  여유 310초. 넘으면 훅이 `exit 124` 를 받아 `"Skipping — commit allowed"` 로 **커밋을 통과시킨다**(무게이트 강등).
  경고는 하므로 침묵은 아니지만, 예산 상향이냐 변경모듈 스코프 분할이냐는 정책 결정.
...
| # | 대상 | 이유 |
|---|---|---|
| 1 | pre-commit 900s 예산 (P1) | 여유 310초. 넘는 순간 게이트가 스스로 무력화된다 |
