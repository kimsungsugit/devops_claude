...
| 14 | `requirements.py::enrich_function_details_with_docs` 미전환 | §5-9. 약함판정 `{"", "inference"}` 가 아직 리터럴이다(`provenance.py` 단일 출처 미사용). 남은 집합이 `WEAK_SOURCES` 의 **진부분집합**이라 과소 덮어쓰기일 뿐 오덮어쓰기는 없다 — 그래서 급하지 않다. 동시 세션이 이 파일을 잡고 있어 미뤘다 |
