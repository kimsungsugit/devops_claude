...
| 11 | X9 (raw fetch silent failure) | frontend 변경 시 `await fetch(` 호출이 (a) `api.js`의 `api/post/postSse` 헬퍼 안 쓰고 raw 호출 + (b) **`authHeaders()` 미부착** + (c) `res.ok` 검사 누락 — 3 조건 충족 시 401/403/500을 silent로 삼키고 success 토스트로 위장하거나 **"경로 오류"·"데이터 0건"으로 위장**. ⚠ **(b) 는 `X-User` 만으론 통과가 아니다** — 커밋 `1b6bb99`(2026-08-04)가 X-User 단독 신원을 차단해 `DEV_MODE_X_USER_FALLBACK=0` 이면 `Authorization: Bearer` 없이는 401 이다. 이 항목이 X-User 만 보던 탓에 raw fetch 12곳이 통째로 401 이 됐고, 그중 `requirements-preview` 가 401 → 요구 0건 → **추적성 매트릭스가 아예 생성되지 않았다**(2026-08-06 사용자 보고). 401 은 로깅 미들웨어보다 **앞에서** 반환돼 백엔드 로그에 요청 줄조차 안 남으므로 로그만 보면 "호출이 없었다"로 오독한다. 자동 가드: `frontend-v2/src/__tests__/RawFetchAuthHeaders.test.jsx`(행동+구조 2층, 뮤테이션 5/5). **점검 명령**: `grep -rn "fetch(" frontend-v2/src --include="*.jsx" --include="*.js"` (결과에서 `api.js:` 정의 라인은 제외) 후 각 호출의 헤더/검사 패턴 검증. `"fetch("`는 BRE 리터럴이라 `await fetch(`·`= fetch(` 모두 매치(과거 `-rnE "…\|…"` 는 ERE에서 `\|`가 리터럴 파이프라 영구 미매치였음 — 표 셀 파이프 회피 위해 alternation 제거). JSON body는 `api()` 헬퍼로 변환, FormData(multipart) 사용 시 raw fetch 정당하지만 `authHeaders()` + res.ok는 명시 필수(multipart 는 `Content-Type` 을 **직접 설정하면 안 된다** — boundary 는 브라우저 몫). 상세: 세션 메모리 `[[feedback_raw_fetch_silent_failure]]` |
