...
| **light** | ≤10줄 단일 코드 파일 (lint/format/오타) **AND** 키워드 트리거 없음 | **기본: reviewer 호출 생략** → 메인 에이전트가 CLAUDE.md `### 작업 종료 직전 비판적 자체 검토`의 **미니 체크리스트 10개** (S1, S3, Q1, R2, X1, X2, X3, X4, X6, X7) 직접 점검. 사용자가 light에서도 reviewer 호출을 명시 요청한 경우에만 reviewer가 실행되며, 이때 점검 항목은 미니 체크리스트와 동일한 10개로 한정 |
...
**예시**: 코드 1파일 8줄 + .md 4파일 변경 → code=light(8줄), doc=meta. 최종 light. 하지만 reviewer는 미니 10개(코드) + 정책 X4/X5/X6(문서) 모두 점검한 결과를 보고한다.
...
- **light** depth → 기본은 reviewer 미호출 (메인이 미니 10개 직접 점검). 명시 요청으로 reviewer가 호출된 경우 미니 10개 항목 — X1/X2/X3/X4/X6/X7 + S1/S3/Q1/R2 — 모두 Pass/Issue로 결정. 나머지는 N/A 허용.
