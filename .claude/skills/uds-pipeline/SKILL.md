---
name: uds-pipeline
description: "**UDS 단일 문서**를 생성합니다 — C 소스 파싱 → AI 분석 → 품질 검증 → DOCX 빌드. STS/SUTS/SITS까지 **4종 순차 + 추적성 검증**이 필요하면 `/doc-pipeline`을 쓰세요."
when_to_use: UDS 생성, UDS만 재생성, 단위설계서, Unit Design Specification, 상세설계서 작성 요청 시
---
