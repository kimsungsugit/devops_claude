...
export function cTypeBoundaries(type) {
  ...
  // ⚠ unsigned(U8/U16/U32)는 0x hex 표기 — 실제 SUTS 시험 내용(주소/데이터/레지스터)이 hex라
  //   경계값 재계산도 같은 진법으로 대조되게 한다(값 자체는 동일: 0x80000000==2147483648).
  //   signed(S8/S16/S32)는 음수 경계가 있어 2의보수 hex가 오히려 헷갈리므로 10진 유지.
  const TABLE = {
