...
export const EMPTY_OUTPUT_TEXT = Object.freeze({
  ...
  // (R38 D-5 ③) 커버리지 문서(SwUT/SwIT)는 TC 수가 아니라 **함수 행**이 내용물이다. 둘 다 0 일
  // 때만 빈 산출물이고, 그 원인은 로그 부재일 수도 **양식 헤더 탐지 실패**일 수도 있다 — 둘을
  // 한 문장에 담아 사용자가 어느 쪽인지 볼 곳을 알게 한다.
  'empty:coverage_rows': '커버리지 행이 0건이라 채점할 것이 없습니다 — 로그 폴더가 비었거나 양식의 헤더를 못 읽은 것입니다. 로그 폴더와 커버리지 양식을 함께 확인하세요.',
});
