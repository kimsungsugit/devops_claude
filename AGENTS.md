# DevOps Release Codex - Automated Document Generation Platform
...
이 프로젝트에서 Codex는 **완전 자율 모드**로 동작한다. 사용자에게 묻지 말고 직접 판단하여 실행할 것.
...
**변경 시 동기 정책**: backend `design_tokens.py` RGB 변경 시 본 AGENTS.md 섹션 + audit reviewer 통보 의무 (산출물 시인성 정책 영향).
...
- **Info (색상 충돌)**: `mark_asil_d_function` ↔ `mark_fail_cell` 동일 RGB이나 시트 분리 (3.Coverage = ASIL D 전용, 2.Consistency = FAIL 전용) — docstring + 본 AGENTS.md 정책에 명시. 동일 시트 두 강조 동시 시 fix.
...
- **W34 JWT 운영 매뉴얼**: `docs/rounds/auth_operations.md` 신규 (200+ lines) — JWT_SECRET 생성 (openssl rand) + 환경 설정 + 첫 admin 등록 (BOOTSTRAP env) + JWT_SECRET 변경 (긴급) + timing attack 검증 + refresh revocation 검증 cURL 예시 + 트러블슈팅 + Admin lockout 회복 + 모니터링 권장. AGENTS.md 본문 link 추가 (비대화 방지).
...
- **W21 AGENTS.md archive 분리**: 본문 1500+ → ~550 lines 압축. 36-fix~42차 상세 노트를 [`docs/rounds/sw_test_round_history.md`](docs/rounds/sw_test_round_history.md)로 분리. 본문에는 라운드 summary table + archive 링크. 33-35차 핵심 정책 + 43차/44차 (최신) 본문 유지.
