...
_TC_PROFILE_EFFECT = {
    "sts": "기본은 요구당 상한까지만 만들어 **매핑된 함수의 대부분이 시험 없이 남습니다**(실측 KJPDS02: 1,037개 중 "
           ...
           "밑에 되풀이됩니다. "
           "**권장**은 TC 수를 그대로 두고 기대결과만 관측 가능하게 바꿉니다 — `기대 결과와 일치`→`반환값: (U16)` · "
           "`{함수} 정상 실행 확인`→`… 실행 후 글로벌 g_X 갱신` · `입력 경계 최솟값 설정`→`경계 최솟값 적용: n=0`, "
           "그리고 하한 위반(`유효 범위 미만`) 스텝을 더합니다(실측 HDPDM01: TC 449 불변, 관측 가능한 기대결과 "
           "34%→60%). 반환 타입도 **쓰기** 전역도 모르는 함수는 문장을 바꾸지 않고 그 수를 공시합니다.",
    "suts": "기본은 전략 24종 상한에 조건 조합 4 · switch 6 · 전역 3 · MC/DC 조건 6 까지입니다. 확장은 기본 시퀀스를 "
            ...
            "모르는 타입은 확장에서도 비웁니다. "
            "**권장**은 SUTS 산출을 바꾸지 않습니다 — 값 칸이 이미 99% 차 있어(실측 61,973칸 중 비움 539) "
            "이번 근거 보강의 대상이 아닙니다.",
    "sits": "기본은 통합 흐름 120 · sub-case 7 까지입니다(실측 KJPDS02: 찾은 흐름 360 중 240 이 빠지고 그중 79 가 "
            ...
            "**권장**은 SITS 산출을 바꾸지 않습니다 — 경계 sub-case 를 낸 흐름 수를 생성 공시에 적을 뿐입니다. "
            "Gen Method 칸에 ABV 를 붙이는 안은 되돌렸습니다: 정본마다 표기 관례가 갈리고(KJPDS02 9% vs "
            "HDPDM01 99%) 붙이려던 문자열이 두 정본 어디에도 없었습니다.",
}
...
def _tc_profile_choice(doc_type: str) -> Dict[str, Any]:
    ...
    """
    from generators.tc_profile import TC_PROFILE_EXTENDED, TC_PROFILE_RECOMMENDED
    return {
        "param": "tc_profile",
        "row": "tc_profile",
        ...
        "options": [
            ...
            # 중간 단계: **물량은 그대로**, 시험 근거만 올린다. 실측(2026-09-20) STS 스텝
            # 8,130행 중 기대결과가 관측 가능한 것이 26%뿐이라 신설했다.
            {"value": TC_PROFILE_RECOMMENDED, "label": "권장 — 물량은 정본 규모, 시험 근거를 보강"},
            {"value": TC_PROFILE_EXTENDED, "label": "확장 — 근거 보강 + 상한 없이"},
        ],
...
def _unmatched_headings_choice() -> Dict[str, Any]:
    ...
    return {
        ...
        "effect": "정본 템플릿의 함수 절 중 **이번 분석에 없는 것**을 어떻게 할지 정합니다. "
                  ...
                  "⏱ 생성 시간도 크게 갈립니다 — **빈 서식을 만드는 일이 소요의 대부분**"
                  "입니다(실측 HDPDM01 정본, 빈 heading 402개: **278초 → 39초**). "
                  "직전 생성이 예산에 가까웠다면 이 선택이 가장 큰 지렛대입니다.",
    }
