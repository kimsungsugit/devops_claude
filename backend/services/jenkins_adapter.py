...
# -----------------------------------------------------------------------------
_KPI_PATTERNS = {
    "FAIL_token": re.compile(r"\bfail(?:ed|ures)?\b", re.IGNORECASE),
    "ERROR_token": re.compile(r"\berror(?:s)?\b", re.IGNORECASE),
    "WARN_token": re.compile(r"\bwarn(?:ing|ings)?\b", re.IGNORECASE),
    "PASS_token": re.compile(r"\bpass(?:ed|es)?\b", re.IGNORECASE),
}

_KPI_NUMERIC_PATTERNS = {
    "FAIL_token": re.compile(r"\bfail(?:ed|ures)?\b[^0-9]{0,12}(\d{1,9})", re.IGNORECASE),
    "ERROR_token": re.compile(r"\berror(?:s)?\b[^0-9]{0,12}(\d{1,9})", re.IGNORECASE),
    "WARN_token": re.compile(r"\bwarn(?:ing|ings)?\b[^0-9]{0,12}(\d{1,9})", re.IGNORECASE),
    "PASS_token": re.compile(r"\bpass(?:ed|es)?\b[^0-9]{0,12}(\d{1,9})", re.IGNORECASE),
}
...
def _extract_kpis(text: str, *, is_html: bool) -> Tuple[Dict[str, int], bool]:
    if not text:
        return {k: 0 for k in _KPI_PATTERNS}, False
    if is_html:
        text = _strip_html(text)
    numeric_used = False
    kpis = {k: 0 for k in _KPI_PATTERNS}
    for key, rx in _KPI_NUMERIC_PATTERNS.items():
        try:
    ...
    if numeric_used:
        return kpis, True
    for key, rx in _KPI_PATTERNS.items():
        try:
            kpis[key] = len(rx.findall(text))
        except Exception:
            kpis[key] = 0
    return kpis, False


def scan_jenkins_build_root(
    build_root: Path,
    progress_cb: Optional[Callable[[str, Dict[str, Any]], None]] = None,
    scan_mode: Optional[str] = None,
    scan_max_files: Optional[int] = None,
) -> Dict[str, Any]:
    """build_root 아래 파일 스캔. 캐시된 결과가 있으면 사용."""
    build_root = Path(build_root)
    max_files = int(getattr(config, "JENKINS_SCAN_MAX_FILES", 6000))
    cache_ttl = int(getattr(config, "JENKINS_SCAN_CACHE_TTL_SEC", 300))
    mode = str(getattr(config, "JENKINS_SCAN_ROOT_MODE", "auto")).strip().lower()
    if scan_mode:
        mode = str(scan_mode).strip().lower()
    if scan_max_files is not None:
        try:
            max_files = max(1, int(scan_max_files))
        except Exception:
            pass

    def _select_scan_root() -> Path:
        report_candidates = [build_root / "report", build_root / "reports"]
        existing = next((p for p in report_candidates if p.exists() and p.is_dir()), None)
        if mode == "report_only":
            return existing or build_root
        if mode == "build_root":
            return build_root
        return existing or build_root

    scan_root = _select_scan_root()

    # 캐시 파일 확인
    cache_path = build_root / "jenkins_scan.json"
    if cache_path.exists():
        try:
            cached_data = json.loads(cache_path.read_text(encoding="utf-8"))
            if isinstance(cached_data, dict) and cached_data.get("root") == str(build_root.resolve()):
                cached_summary = cached_data.get("summary") if isinstance(cached_data.get("summary"), dict) else {}
                if "kpi_mode" not in cached_summary:
                    raise ValueError("kpi_mode missing in cache")
                if cached_summary.get("scan_root_mode") != mode:
                    raise ValueError("scan_root_mode mismatch")
                if cached_summary.get("scan_max_files") != max_files:
                    raise ValueError("scan_max_files mismatch")
                if cache_ttl > 0:
                    age = datetime.now().timestamp() - cache_path.stat().st_mtime
                    if age <= cache_ttl:
                        if progress_cb:
                            try:
                                progress_cb("scan_cached", {"age_sec": int(age)})
                            except Exception:
                                pass
                        return cached_data
                else:
                    if progress_cb:
                        try:
                            progress_cb("scan_cached", {"age_sec": 0})
                        except Exception:
                            pass
                    return cached_data
        except Exception:
    ...
    yield_every = int(getattr(config, "JENKINS_SCAN_YIELD_EVERY", 200))
    yield_sleep = float(getattr(config, "JENKINS_SCAN_YIELD_SLEEP_MS", 5)) / 1000.0
    scanned = 0
    truncated = False
    if progress_cb:
        try:
            progress_cb("scan_prepare", {"max_files": max_files})
        except Exception:
            pass
    for p in scan_root.rglob("*"):
        ...
        if max_files > 0 and scanned > max_files:
            truncated = True
            break
        if yield_every > 0 and scanned % yield_every == 0:
            try:
                time.sleep(max(0.0, yield_sleep))
            except Exception:
                pass
        if progress_cb and scanned % 200 == 0:
            try:
                rel_prog = str(p.relative_to(build_root)).replace("\\", "/")
                progress_cb(
                    "scan_files",
                    {"current": scanned, "max_files": max_files, "file": rel_prog},
                )
            except Exception:
                pass
        ...
        if low.endswith((".html", ".htm")):
            ...
            kpis, used_numeric = _extract_kpis(txt, is_html=True)
            if used_numeric:
                numeric_kpi_used = True
            ...
            kpis, used_numeric = _extract_kpis(txt, is_html=False)
            if used_numeric:
                numeric_kpi_used = True
            kpis_by_file[rel] = kpis
    ...
    result = {
        ...
        "summary": {
            ...
            "kpi_mode": "numeric" if numeric_kpi_used else "token",
        },
    }
    ...
    if progress_cb:
        try:
            progress_cb("scan_done", {"scanned": scanned, "truncated": truncated})
        except Exception:
            pass
    return result
...
def _scan_runtime_summary(build_root: Path, jscan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    limit = int(getattr(config, "JENKINS_RUNTIME_LOG_SCAN_LIMIT", 6))
    scan_logs = logs[: max(1, limit)]
...
def _table_matrix(table) -> Tuple[List[str], List[List[str]]]:
    ...
    if rows and all(
        (rows[0][i] or "").strip().upper() == (norm_headers[i] or "").strip().upper()
...
def _summarize_vcast_tests(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    total = len(rows)
    passed = 0
    failed = 0
    skipped = 0
    unknown = 0
    for row in rows:
        status = _normalize_vcast_result(row.get("result"))
        if status == "pass":
            passed += 1
        elif status == "fail":
            failed += 1
...
def _extract_vcast_test_rows(
    ...
    if len(tables) > max_tables:
        errors.append("tables_truncated")
    return rows_out, errors, truncated
...
def _write_jenkins_scan_bundle(reports_dir: Path, jscan: Dict[str, Any]) -> None:
    ...
    safe_scan = jscan if isinstance(jscan, dict) else {}
    _write_json(bundle_dir / "jenkins_scan.json", safe_scan)
    _write_markdown(bundle_dir / "jenkins_scan.md", _summarize_jenkins_scan_md(safe_scan))
...
def _write_vectorcast_rag_bundle(
    *,
    ...
    max_rows = int(getattr(config, "VCAST_TEST_ROWS_MAX_ROWS", 5000))
    max_tables = int(getattr(config, "VCAST_TEST_ROWS_MAX_TABLES", 6))
    for p in ut_reports:
        if p:
            rows, errors, truncated = _extract_vcast_test_rows(p, max_rows=max_rows, max_tables=max_tables)
            parse_errors += errors
            truncated_rows = truncated_rows or truncated
            for row in rows:
                row["source"] = "UT"
    ...
    for row in test_rows:
        if _normalize_vcast_result(row.get("result")) == "fail":
            failures.append(
                {
    ...
    _write_markdown(
        bundle_dir / "vectorcast_rag.md",
        _summarize_vcast_rag_md(
            {
...
def parse_vcast_execution_result_report(path: Path) -> Dict[str, Any]:
    ...
    config_tbl = _table_kv(tables[0]) if tables else {}
    ...
    return {"ok": True, "path": str(path), "config": config_tbl, "testcases": testcases}
...
def parse_vcast_metrics_simple_report(path: Path) -> Dict[str, Any]:
    ...
    return {"ok": True, "path": str(path), "config": config_tbl, "entries": entries}
...
def write_complexity_csv(entries: List[Dict[str, Any]], out_csv: Path) -> None:
    """프론트 '복잡도' 탭에서 읽는 complexity.csv 생성 (lizard 스타일)."""
...
def parse_prqa_summary_html(path: Path) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {
        "ok": True,
        "path": str(path),
        "kind": (soup_obj.title.string.strip() if soup_obj.title and soup_obj.title.string else ""),
    }
    kv: Dict[str, Any] = {}
...
def parse_prqa_his_metrics_xlsx(path: Path, *, top_n: int = 30, max_rows: int = 5000) -> Dict[str, Any]:
    ...
    try:
        ...
        fs = (
            g.agg(functions=("function", "count"), vg_max=("vg", "max"), vg_mean=("vg", "mean"), calls_sum=("calls", "sum"))
            .reset_index()
        )
...
def _flatten_scan_candidates(jscan: Dict[str, Any]) -> List[str]:
    files = jscan.get("files") if isinstance(jscan, dict) else None
...
def _find_one_from_candidates(
    ...
    if not candidates:
        return None
    matches: List[Path] = []
    pats = [p.strip() for p in patterns if p and str(p).strip()]
    if not pats:
        return None
    for rel in candidates:
        ...
        for pat in pats:
            ...
            if fnmatch.fnmatch(rel_low, pat_norm.lower()):
                cand = (Path(build_root) / rel_norm).resolve()
                if cand.exists():
                    matches.append(cand)
                break
    if not matches:
...
def ensure_frontend_summary(
    *,
    ...
    """Jenkins 동기화 이후 최소 파일 생성 + 리포트 요약."""
    reports_dir = Path(reports_dir)
    build_root = Path(build_root)

    jscan = scan_jenkins_build_root(
        build_root,
        progress_cb=progress_cb,
        scan_mode=scan_mode,
        scan_max_files=scan_max_files,
    )
    if not isinstance(jscan, dict):
        jscan = {}
    ...
    scan_candidates = _flatten_scan_candidates(jscan) if isinstance(jscan, dict) else []
    def _find_report(patterns: List[str]) -> Optional[Path]:
        hit = _find_one_from_candidates(build_root, scan_candidates, patterns)
        if hit:
            return hit
        return _find_one_in_roots(search_roots, patterns)

    ut_full = _find_report(["*UT*_full_report.html", "*_UT_*_full_report.html", "*UT_full_report.html", "*UT*_full*.html"])
    ut_metrics = _find_report(["*UT*_metrics_report.html", "*_UT_*_metrics_report.html", "*UT_metrics_report.html", "*UT*_metrics*.html"])
    ut_env = _find_report(["*UT*_environment_report.html", "*_UT_*_environment_report.html", "*UT_environment_report.html", "*UT*_environment*.html"])

    it_full = _find_report(["*IT*_full_report.html", "*_IT_*_full_report.html", "*IT_full_report.html", "*IT*_full*.html"])
    it_metrics = _find_report(["*IT*_metrics_report.html", "*_IT_*_metrics_report.html", "*IT_metrics_report.html", "*IT*_metrics*.html"])
    it_env = _find_report(["*IT*_environment_report.html", "*_IT_*_environment_report.html", "*IT_environment_report.html", "*IT*_environment*.html"])

    agg_cov = _find_report(["*AggregateCoverageReport.html", "*AggregateCoverage*.html"])
    exec_res = _find_report(["*ExecutionResultReport.html", "*ExecutionResult*.html"])
    metrics_rep = _find_report(["*MetricsReport.html", "*MetricsReport*.html"])
    tc_data = _find_report(["*TestCaseDataReport.html", "*TestCaseData*.html"])
    ...
    prqa: Dict[str, Any] = {}
    crr = _find_report(["*_CRR_*.html", "*CRR*.html"])
    rcr = _find_report(["*_RCR_*.html", "*RCR*.html"])
    hmr_xlsx = _find_report(["*_HMR_*.xlsx", "*HMR*.xlsx"])
