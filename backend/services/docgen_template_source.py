...
| | 표준 템플릿 | 정본(고치기 전) | 정본(고친 후) |
|---|---|---|---|
| 소요 | 11초 | 1,116.5초 | **809.4초**(13분 29초) |
| payload 함수 반영 | **0/57 (0.0%)** | 57/57 | **57/57 (100.0%)** |
| SwUFn heading | 8개(자리표시자) | 1,035개 | 1,035개(실제 함수명) |
| 표지(body 직속 `w:sdt`) | — | **유실** | **보존**(`KJPDS02 / v2.08`) |
| 이력 표 데이터 행 | — | **0행** | **20행** |
| 콘텐츠 컨트롤 값 | — | **유실** | 보존(+`[Project Name]` 채움) |

처음 실측에서는 **표지·이력·컨트롤 값이 셋 다 사라졌다** — 즉 정본을 쓰는 이유로 든
문장이 docx 에서는 거짓이었다. 원인은 재작성 경로 셋이었고(`_iter_template_blocks` 가
body 직속 `w:sdt` 를 안 냄 / 표를 모양으로만 담음 / `p.text` 가 `w:sdt` 안을 못 읽음)
`report_gen/docx_builder.py` 에서 고쳤다. 지금은 **표지·이력·Introduction 이 실제로
정본과 같다**. 빈 표를 새로 만들지 않게 되어 소요도 27% 줄었다.

그래도 문구를 문서 종류별로 가른다 — docx 에서 정본이 주는 **가장 큰 것**은 표지가
아니라 **함수 heading 집합**이기 때문이다(반영률이 0% ↔ 100% 로 갈린다). 시트 빌더에는
그 축이 없다.
"""
...
def _reference_gain(kind: str) -> str:
    ...
    return ("표지·이력·Introduction 이 정본과 같아지고, **함수 heading 집합**도 정본의 "
            "것을 씁니다 — 그래서 **이 프로젝트 함수가 문서에 실립니다**(실측: 표준 "
            "템플릿은 57개 중 0개, 정본은 57개 전부). 명세 본문은 새로 씁니다.")
...
def _reference_caveat(kind: str) -> str:
    ...
    return (" ⚠ 표지의 프로젝트명·문서 버전과 이력 표가 **정본 그대로** 옵니다"
            "(실측: 표지 `v2.08`, 이력 20행) — 새 개정으로 낼 때는 그 두 곳을 갱신하세요. "
            "그리고 정본이 크면 오래 걸립니다: 53MB 정본으로 13분 29초, 표준 템플릿은 "
            "11초였습니다.")
