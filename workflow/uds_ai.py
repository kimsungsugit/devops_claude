...
def _build_func_desc_prompt(batch: List[Dict[str, Any]], *, pass_num: int = 1) -> str:
    if pass_num == 2:
        ...
        lines = [
            ...
            "Follow the style of these reference examples from automotive UDS documents:",
            "",
            "Reference examples (notice: timing, caller, sub-function calls are specified):",
            '{"init_func": "POR(Power On Reset) 시 호출되어 시스템을 초기화한다. 초기화 후 순차적으로 하위 Task 함수들을 호출하며 시스템 동작을 수행한다.",',
            ' "periodic_ctrl": "System Tick Timer에 의해 1ms마다 호출되어 Watchdog Input 값을 Toggle 시켜 WDI 기능을 수행한다.",',
            ' "driver_input": "주기적으로(5ms) 호출되어 ADC 측정값을 읽고, 센서 데이터를 드라이브단에 저장한다.",',
            ' "app_control": "주기 함수에 의해 5ms마다 호출되어, 상태에 따른 제어 동작을 결정하고 액추에이터를 구동한다."}',
            "",
